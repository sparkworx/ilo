...
PROMPT while connected to the SYS account with SYSDBA privileges.
PROMPT
...
PROMPT Using argument 1: &1 as owner of ILO packages
...
set echo off heading off termout off feedback off
select 'start ' || 'ilo_userok.sql'
from all_users where upper(username) = upper('&&h_user')
UNION
select 'start ' || 'ilo_usernotok.sql'
from dual 
